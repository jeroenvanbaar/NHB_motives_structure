...
function basis = Create_random_basis_regret(varargin)
...
%         pause(.5);
    ...
    boundary_elements = find(imdilate(grid,strel_use) - grid);
    for i = 1:4
        bound_el_index = randi(length(boundary_elements));
        grid(boundary_elements(bound_el_index)) = .5;
        boundary_elements = boundary_elements(boundary_elements ~= boundary_elements(bound_el_index));
